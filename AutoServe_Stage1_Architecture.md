# AutoServe — Stage 1 Architecture (legacy root copy)

> **Canonical doc:** `docs/ARCHITECTURE.md`. Prefer that file.
> Stage 1 roles in code: **owner | staff**. AI: **Groq**.
...
Stack: Next.js (TypeScript) frontends, Supabase (Postgres, Auth, Storage, Edge Functions) as the backend, Groq API for the AI assistant feature (called server-side only, from an Edge Function — never from the browser). Hosting: Vercel (frontend) + Supabase (backend), both free tier. Real, unavoidable costs at this scale: a domain name (~$10–15/year) and Groq API usage (usage-based, not free — see section 6c for how this is scoped down to keep it cheap).
